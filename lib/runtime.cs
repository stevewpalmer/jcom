...
namespace JComLib {
    ...
    public static class Runtime {
        ...
        public static string ReadLine(string existingLine, int maxWidth = 0, bool newlineAtEnd = true) {

            StringBuilder buffer = new(existingLine);
            int cursorPosition = Console.CursorLeft;
            int bufferIndex = 0;

            if (!string.IsNullOrEmpty(existingLine)) {
                Console.Write(existingLine);
                cursorPosition += existingLine.Length;
                bufferIndex = existingLine.Length;
            ...
            while (keyInfo.Key != ConsoleKey.Enter) {
                switch (keyInfo.Key) {
                    ...
                    default:
                        if (!char.IsControl(keyInfo.KeyChar) && (maxWidth == 0 || buffer.Length < maxWidth)) {
